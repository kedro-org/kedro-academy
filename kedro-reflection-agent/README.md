...
LLM setup uses Kedro [LLM context nodes](https://docs.kedro.org/en/stable/build/llm_context_node/): each pipeline builds an `LLMContext` from `ChatOpenAIDataset` + prompt datasets, and execution nodes call `llm_invoke`. Catalog is split per pipeline: `catalog_agent_run.yml`, `catalog_evaluation.yml`, `catalog_reflection.yml`, `catalog_apply.yml`.
...
See `conf/local/credentials.template.yml` for variable names.

```bash
cd kedro-reflection-agent
