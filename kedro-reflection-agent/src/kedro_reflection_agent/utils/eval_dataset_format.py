"""Convert between eval cases and Langfuse dataset item format."""
...
def to_langfuse_items(cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Map eval cases to LangfuseEvaluationDataset item schema (requires ``input``)."""
    items = []
    for case in cases:
...
def from_langfuse_items(items: list[dict[str, Any]] | Any) -> list[dict[str, Any]]:
    """Extract eval cases from Langfuse items or DatasetClient."""
