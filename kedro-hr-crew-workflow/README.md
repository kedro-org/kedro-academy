...
### Kedro vs CrewAI Flows

[CrewAI 1.x](https://docs.crewai.com/v1.15.2/en/introduction) recommends using **Flows** for orchestration and **Crews** for multi-agent work within each step. This project intentionally uses **Kedro pipelines as the Flow layer** and **CrewAI Crews inside Kedro nodes** for agent collaboration.

| Layer | CrewAI-native pattern | This project |
| --- | --- | --- |
| Orchestration | CrewAI Flow | Kedro pipelines (`jobs` → `applications` → `screening` → `reporting`) |
| Multi-agent work | Crew within a Flow step | CrewAI `Crew` in pipeline nodes |
| Prompts & config | YAML / project files | Kedro catalog + `llm_context_node` |

**Requirements:** Python 3.10–3.13, Kedro 1.3.x, CrewAI 1.15.x.
...
└────────────────────────┬────────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────────┐
│ Screening Pipeline (Agentic — 2 phases)                         │
│   Phase 1: RequirementsMatcherAgent → match_results             │
│   Deterministic scoring → match_score, must_have_coverage     │
│   Phase 2: ResumeEvaluatorAgent → recommendation + narrative    │
...
4. **Two-phase screening**: Matcher agent finds evidence; scores are computed in code; evaluator agent recommends using pre-computed scores plus qualitative analysis

5. **Scoring vs recommendation split**: Numeric scores from `calculate_scoring_result()`; LLM drafts recommendation and narrative; `apply_recommendation_guardrails()` enforces threshold rules on the final label
...
```bash
# Process default resume (ds.docx via resume_id=ds)
kedro run
...
```bash
# Edit data/config/reporting/email_templates.yml
proceed:
...
│   │   ├── catalog_genai.yml    # LLM and prompt configurations
...
Edit `conf/base/catalog_genai.yml`:
