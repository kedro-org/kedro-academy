...
def extract_structured_task_output(
    crew_result: Any,
    model_class: type[T],
) -> dict[str, Any]:
    """Extract validated structured output from the final crew task.

    Prefers CrewAI pydantic task outputs and falls back to JSON parsing.
    """
    ...
    if hasattr(final_output, "pydantic") and final_output.pydantic is not None:
        return get_model_dump(
            model_class,
            **final_output.pydantic.model_dump(mode="json"),
        )

    if hasattr(crew_result, "pydantic") and crew_result.pydantic is not None:
        return get_model_dump(
            model_class,
            **crew_result.pydantic.model_dump(mode="json"),
        )

    parsed = parse_json_from_text(str(final_output))
    if not parsed or not isinstance(parsed, dict):
        raise ValueError(
            f"Failed to parse structured output from task result: {str(final_output)[:200]}"
        )
