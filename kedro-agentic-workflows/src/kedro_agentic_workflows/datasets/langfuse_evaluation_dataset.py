...
class LangfuseEvaluationDataset(AbstractDataset[list[dict[str, Any]], "DatasetClient"]):
    ...
    **On load / save behaviour:**
    ...
    - **On save:** Upserts all items to the remote dataset — items with an
      existing ``id`` are updated in place, new items are created. In
      ``local`` mode, also merges them into the local file. In ``remote``
      mode, only the remote upload occurs.
    ...
    **Sync policies:**
    ...
    - **local** (default): The local file is the source of truth. On
      ``load()``, all local items are upserted to remote — items with an
      existing ``id`` are updated, new items are created. Items without
    ...
    - **remote**: The remote Langfuse dataset is the sole source of truth.
      ...
      interaction. ``save()`` upserts items to remote but does not write
      to any local file. An optional
    ...
    def __init__(
        ...
        Args:
            ...
            sync_policy: Controls the source of truth for reads and whether
                ...
                ``"local"`` (default) — local file is upserted to remote on
                ``load()``; ``save()`` upserts to remote and merges into
                the local file (new data takes precedence).
