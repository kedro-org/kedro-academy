...
class LangfuseTraceDataset(AbstractDataset):
    ...
    def _validate_langfuse_credentials(self) -> None:
        ...
        for key in OPTIONAL_LANGFUSE_CREDENTIALS:
            if key in self._credentials:
                ...
                if (
                    not self._credentials[key]
                    or not str(self._credentials[key]).strip()
                ):
                    raise ValueError(
    ...
    def _build_openai_client_params(self) -> dict[str, str]:
        ...
        if (
            not openai_creds["openai_api_key"]
            or not openai_creds["openai_api_key"].strip()
        ):
    ...
    def save(self, data: Any) -> None:
        ...
        raise NotImplementedError(
            "LangfuseTraceDataset is read-only - it provides tracing clients, not data storage"
        )
